    ...
    $('.uploader-group').each(function(index) {
            ...
            function addFile(file, now_uploader) {
                now_uploader.makeThumb(file, function(error, src) {
                    append_str = '<div id="' + file.id + '" class="item img-item pull-left" style="margin-bottom:10px;margin-right:10px;">';
                    if(extensions.lastIndexOf('png') >= 0)
                    {
                        append_str += '<img style="width:100px;height:100px;" src="' + src + '">';
                    }
                    else
                    {
                        append_str += '<video width="320" height="240" controls><source src="' + src + '" type="video/mp4"></video>';
                    }
                    append_str += '<i class="fa fa-close close-btn"></i><p class="state">等待上传...</p></div>';
                    $(".uploader-picker").eq(index).siblings(".uploader-list").append(append_str);
                });
            }
            ...
            uploader[index].on('uploadSuccess', function(file, response) {
                ...
                var name = $('#' + file.id).parents(".uploader-list").siblings("a").data("file"),
                    rna = response.data.realname,
                    val = response.data.filename,
                    src = response.data.filelink;
                var str = '<input type="hidden" value="' + val + '" name="' + name + '[filename][]" class="hid-filename ">';
                    str += '<input type="hidden" value="' + src + '" name="' + name + '[filelink][]" class="hid-filename ">';
                    str += '<input type="hidden" value="' + rna + '" name="' + name + '[realname][]" class="hid-filename ">';
                $('#' + file.id).append(str);
                ...
                {
                    $('#' + file.id).find("video").attr("src", src);
                }
