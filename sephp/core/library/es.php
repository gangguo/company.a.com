...
require_once dirname(__FILE__).'/../../../common/vendor/autoload.php';
...
class cls_es
{
     ...
     * @param null   $value
     * @param string $leaf
     * @param string $boolean and or not 三种分别对应 must should must_not
     * @return $this
     */
     ...
     * @return array
     */
    public function put_mapping($index_name, array $params)
    {
        ...
        !empty($this->type) && $params['type'] = $this->type;//7.0版本的es是不需要type的
        ...
        try
        {
            ...
            switch ($this->_type)
            {
                case 'select':
                    ...
                    if(isset($return['total']['value']))  $return['total'] = $return['total']['value'];
                    if ( $return['total'] > 0 )
                    {
                        foreach($result['hits']['hits'] as $row)
                        {
                            $other = ['id' => $row['_id'], '_score' => util::get_value($row, '_score')];
                            if(isset($row['highlight']))
                            {
                                $other['highlight'] = $row['highlight'];
                            }
                            $tmp[] = array_merge($other, $row['_source']);
                        }
                    }
            ...
            foreach ($this->expr as $key => $item)
            {
                if ( !is_numeric($key) )
                {
                    $body = util::array_merge_multiple(
                        $body,
                        $this->compile_muti_array($key, $item) //此处会放到下面query层
                    );
                }
    ...
    public function get_compiled_insert($body)
    {
        ...
        {
            $data = reset($this->_set);
            return array_merge(
                array_merge(['body' => $data, 'id' => $data['id']], $body),
                ['method' => 'create']
            );
        }
    }
    ...
    protected function _compile_where(): array
    {
        ...
        foreach ($where_groups as $where)
        {
            ...
            foreach ($where as $where)
            {
                ...
                elseif( $where['boolean'] == 'not' )
                {
                    $must_not[] = $this->_compilee_where_leaf(
                        $where['leaf'], $where['column'],
                        $where['operator'], $where['value']
                    );
                }
            ...
            if ( $must )
            {
                if($operation == 'must')
                {
                    $bool['bool']['must'] = $must;//只有and的时候$where_groups就一个元素
                }
                else
                {
            ...
            if( $must_not )
            {
                $bool['bool'][$operation][] = ['bool' => ['must_not' => $must_not]];
            }
        }
    ...
    protected function _compile_where_group(array $where): array
    {
        ...
        if( !empty($or_index) )
        {
            $group[] = array_slice($where, $last_index, 1);
            $group[] = array_slice($where, $last_index + 1);
        }
        else
        {
            $group[] = array_slice($where, $last_index);
        }
